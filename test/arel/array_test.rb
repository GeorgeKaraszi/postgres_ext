...
describe "Array Column Predicates" do
  ...
  describe "Array Overlap" do
    ...
    it "returns matched records" do
      ...
      query = arel_table.where(arel_table[:tags].overlap(%w[two one])).project(Arel.sql("*"))
      Person.find_by_sql(query.to_sql).must_include(two)
      Person.find_by_sql(query.to_sql).must_include(one)
    end
  end
  ...
  describe "Array Contains" do
    ...
    it "returns matched records" do
      one = Person.create!(tags: %w[one two three])
      two = Person.create!(tags: %w[one three])

      query = arel_table.where(arel_table[:tags].contains(%w[one two])).project(Arel.sql("*"))
      Person.find_by_sql(query.to_sql).must_include one
      Person.find_by_sql(query.to_sql).wont_include two

      query = arel_table.where(arel_table[:tags].contains(%w[one three])).project(Arel.sql("*"))
      Person.find_by_sql(query.to_sql).must_include one
      Person.find_by_sql(query.to_sql).must_include two
  ...
  describe "Array Contained In Array" do
    ...
    it "returns matched records" do
      one = Person.create!(tags: %w[one two])
      two = Person.create!(tags: %w[one three])

      query = arel_table.where(arel_table[:tags].contained_in_array(%w[one two three])).project(Arel.sql("*"))
      Person.find_by_sql(query.to_sql).must_include one
      Person.find_by_sql(query.to_sql).must_include two

      query = arel_table.where(arel_table[:tags].contained_in_array(%w[one three])).project(Arel.sql("*"))
